...
describe('SkillsManager', function() {
  ...
  describe('loadSkill()', () => {
    it('should load skill successfully', async () => {
      ...
      assert.strictEqual(result.executionCount, 0);
    });
    ...
    it('should throw error for invalid skill name', async () => {
      try {
        await manager.loadSkill('');
        assert.fail('Should have thrown error');
      } catch (error) {
        assert.ok(error.message.includes('Skill name must be'));
      }
    });

    it('should throw error for non-existent skill', async () => {
      try {
        await manager.loadSkill('non-existent-skill');
        assert.fail('Should have thrown error');
      } catch (error) {
        assert.ok(error.message.includes('not found'));
      }

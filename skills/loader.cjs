...
class SkillLoader {
  ...
  async loadSkill(skillName) {
    ...
    try {
      ...
      console.log(`[LOADER DEBUG] Loading skill '${skillName}' from: ${entryPoint}`);
      // Use createRequire para garantir contexto CommonJS correto
      const skillRequire = createRequire(path.join(skillPath, 'package.json'));
      let skillModule;
      try {
        skillModule = skillRequire(entryPoint);
      } catch (requireError) {
        console.error(`[LOADER ERROR] Failed to require '${entryPoint}':`);
        console.error('Error message:', requireError.message);
        console.error('Stack trace:', requireError.stack);
        throw requireError;
      }
  ...
  _resolveSkillPath(skillName) {
    ...
    // Caminho base do projeto (usa path do registry.json como referência)
    const registryDir = path.dirname(require.resolve('./registry.json'));
    const basePath = path.resolve(registryDir, '..');
